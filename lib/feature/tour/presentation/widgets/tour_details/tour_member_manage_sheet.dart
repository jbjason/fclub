...
class TourMemberManageSheet extends StatefulWidget {
  const TourMemberManageSheet({super.key});

  @override
  State<TourMemberManageSheet> createState() => _TourMemberManageSheetState();
}

class _TourMemberManageSheetState extends State<TourMemberManageSheet> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        context.read<TourEventProvider>().loadAvailableParticipants();
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final available = provider.availableParticipants;
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                Icon(Icons.people_alt_rounded, color: Colors.white, size: 20.r),
                SizedBox(width: 10.w),
                Expanded(
                  child: Column(
                ...
                if (available.isNotEmpty) ...[
                  ...
                  ...available.map(
                    (c) => _ContactTile(
                      ...
                      onAdd: () => _run(
                        () => context.read<TourEventProvider>().addParticipant(
                          c.id,
                        ),
                      ),
                    ),
